...
class SOCIAL_HASHTAG_CACHE {
  ...
  function build_api_search_url($option_num=0){
    ...
    $query = $api_options['api_scheme'] . "://" . $api_options['api_host'];
                                               //| http://v1.api.social_hashtag.com
    ...
    //this is for the oauth for twitter. the query should only consist of these things. authentication is done separately.
    if( $api_options['oauth_type']){

      $query .= "/" . str_replace("%string%", urlencode($api_options['string']), $api_options['api_endpoint']);  //https://v1.api.social_hashtag.com:8080/search?string=xxxx&apikey=xxxxxxxxxx
    }
    ...
    if( $api_settings['api_selected'] == 'twitter' ) {
      $query.= "&count=100";
      $query.= "&result_type=mixed&include_entities=1";
  ...
  function oauth($url){
    //since twitter oauth returns differently we have to get the contents differently and authenticate in this function.
      include_once("oauth.php");//variables for authentication-- api key and app token as $app_key and $app_token
      //query right now is https://api.twitter.com/
      ...
      return file_get_contents($api_base.'1.1/search/tweets.json?q=brooklyn',false,$context);
